...
//        })
        $this->setLabel($label);
        $this->setPath($path);
        $this->setView('admin::forms.translatabletext');
...
//                $this->getName().':en' => 'required|max:5',
//                $this->getName().':ru' => 'required|max:5',
//                $this->getName().':ua' => 'required|max:5',
//            ]);
//            dd($validatedData);
            foreach (Language::all() as $language) {
                $model->{$this->getName().':'.$language->code} = Input::get()[$this->getName().':'.$language->code];
            }
            //  $model->save();
        });
...
////        $this->required = true;
////        $this->setData( $this->data + ['required'=> true]);
...
//
//
//            $messages[str_replace($this->getName(),$this->getName().':en',$rule)] = $message;
//            unset($messages[$rule]);
//        }
////        $rule = str_replace($this->getName(),$this->getName().':en',$messages);
////            dd($messages);
//        return $messages;
//    }
    public function getValidationMessages()
